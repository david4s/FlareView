//
//  DCProgressHUD.h
//  DCProgressHUD, https://github.com/DCProgressHUD/DCProgressHUD
//
//  Copyright (c) 2011-2017 Sam Vermette and contributors. All rights reserved.
//

#if !__has_feature(objc_arc)
#error DCProgressHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "DCProgressHUD.h"
#import "DCIndefiniteAnimatedView.h"
...
NSString * const DCProgressHUDDidReceiveTouchEventNotification = @"DCProgressHUDDidReceiveTouchEventNotification";
NSString * const DCProgressHUDDidTouchDownInsideNotification = @"DCProgressHUDDidTouchDownInsideNotification";
NSString * const DCProgressHUDWillDisappearNotification = @"DCProgressHUDWillDisappearNotification";
NSString * const DCProgressHUDDidDisappearNotification = @"DCProgressHUDDidDisappearNotification";
NSString * const DCProgressHUDWillAppearNotification = @"DCProgressHUDWillAppearNotification";
NSString * const DCProgressHUDDidAppearNotification = @"DCProgressHUDDidAppearNotification";

NSString * const DCProgressHUDStatusUserInfoKey = @"DCProgressHUDStatusUserInfoKey";

static const CGFloat DCProgressHUDParallaxDepthPoints = 10.0f;
static const CGFloat DCProgressHUDUndefinedProgress = -1;
static const CGFloat DCProgressHUDDefaultAnimationDuration = 0.15f;
static const CGFloat DCProgressHUDVerticalSpacing = 12.0f;
static const CGFloat DCProgressHUDHorizontalSpacing = 12.0f;
static const CGFloat DCProgressHUDLabelSpacing = 8.0f;


@interface DCProgressHUD ()
...
- (void)dismiss;
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(DCProgressHUDDismissCompletion)completion;
...
+ (DCProgressHUD*)sharedView {
    static dispatch_once_t once;
    
    static DCProgressHUD *sharedView;
...
+ (void)setDefaultStyle:(DCProgressHUDStyle)style {
    [self sharedView].defaultStyle = style;
    [self sharedView].hudView.alpha = style != DCProgressHUDStyleCustom ? 1.0f : 0.0f;
}

+ (void)setDefaultMaskType:(DCProgressHUDMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(DCProgressHUDAnimationType)type {
...
+ (void)setForegroundColor:(UIColor*)color {
    ...
    [self setDefaultStyle:DCProgressHUDStyleCustom];
}
...
+ (void)setLoadingViewHide:(BOOL)ishide {
    [self sharedView].indefiniteAnimatedView.hidden = ishide;
}

#pragma mark - Show Methods

+ (void)show {
    [self setLoadingViewHide:NO];
    [self showWithStatus:nil];
}

+ (void)showWithMaskType:(DCProgressHUDMaskType)maskType {
    DCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
...
+ (void)showWithStatus:(NSString*)status {
    [self setLoadingViewHide:NO];
    [self showProgress:DCProgressHUDUndefinedProgress status:status];
}

+ (void)showCompletedContent:(NSString*)status {
    [self setLoadingViewHide:YES];
    [self showProgress:DCProgressHUDUndefinedProgress status:status];
}

+ (void)showProgressHUDCompleteMessage:(NSString*)content Duration:(float)secord{
    [self showCompletedContent:content];
    [self dismissWithDelay:secord];
}

+ (void)showWithStatus:(NSString*)status maskType:(DCProgressHUDMaskType)maskType {
    DCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress maskType:(DCProgressHUDMaskType)maskType {
    DCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress status:(NSString*)status maskType:(DCProgressHUDMaskType)maskType {
    DCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showInfoWithStatus:(NSString*)status maskType:(DCProgressHUDMaskType)maskType {
    DCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showSuccessWithStatus:(NSString*)status maskType:(DCProgressHUDMaskType)maskType {
    DCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showErrorWithStatus:(NSString*)status maskType:(DCProgressHUDMaskType)maskType {
    DCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showImage:(UIImage*)image status:(NSString*)status maskType:(DCProgressHUDMaskType)maskType {
    DCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)dismissWithCompletion:(DCProgressHUDDismissCompletion)completion {
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        _defaultMaskType = DCProgressHUDMaskTypeNone;
        _defaultStyle = DCProgressHUDStyleLight;
        _defaultAnimationType = DCProgressHUDAnimationTypeFlat;
        _minimumSize = CGSizeZero;
        _font = [UIFont preferredFontForTextStyle:UIFontTextStyleSubheadline];
        
        NSBundle *bundle = [NSBundle bundleForClass:[DCProgressHUD class]];
        NSURL *url = [bundle URLForResource:@"DCProgressHUD" withExtension:@"bundle"];
        ...
        _fadeInAnimationDuration = DCProgressHUDDefaultAnimationDuration;
        _fadeOutAnimationDuration = DCProgressHUDDefaultAnimationDuration;
        ...
        self.accessibilityIdentifier = @"DCProgressHUD";
...
- (void)updateHUDFrame {
    ...
    if(imageUsed || progressUsed) {
        if (self.indefiniteAnimatedView.hidden) {
    ...
    hudWidth = DCProgressHUDHorizontalSpacing + MAX(labelWidth, contentWidth) + DCProgressHUDHorizontalSpacing;
    
    // |-spacing-content-(labelSpacing-label-)spacing-|
    hudHeight = DCProgressHUDVerticalSpacing + labelHeight + contentHeight + DCProgressHUDVerticalSpacing;
    if(self.statusLabel.text && (imageUsed || progressUsed)){
        // Add spacing if both content and label are used
        hudHeight += DCProgressHUDLabelSpacing;
    }
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(DCProgressHUDVerticalSpacing, (self.minimumSize.height - contentHeight - DCProgressHUDLabelSpacing - labelHeight) / 2.0f);
    ...
    if(self.progress != DCProgressHUDUndefinedProgress) {
    ...
    if(imageUsed || progressUsed) {
        if (self.indefiniteAnimatedView.hidden){
            centerY = DCProgressHUDVerticalSpacing+DCProgressHUDLabelSpacing + labelHeight / 2.0f;
        }
        else{
            centerY = CGRectGetMaxY(imageUsed ? self.imageView.frame : self.indefiniteAnimatedView.frame) + DCProgressHUDLabelSpacing + labelHeight / 2.0f;
        }
    }
    else {
        centerY = CGRectGetMidY(self.hudView.bounds);
...
- (void)updateMotionEffectForXMotionEffectType:(UIInterpolatingMotionEffectType)xMotionEffectType yMotionEffectType:(UIInterpolatingMotionEffectType)yMotionEffectType {
    ...
    effectX.minimumRelativeValue = @(-DCProgressHUDParallaxDepthPoints);
    effectX.maximumRelativeValue = @(DCProgressHUDParallaxDepthPoints);
    
    UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:yMotionEffectType];
    effectY.minimumRelativeValue = @(-DCProgressHUDParallaxDepthPoints);
    effectY.maximumRelativeValue = @(DCProgressHUDParallaxDepthPoints);
...
#else
            // If DCProgressHUD is used inside an app extension add it to the given view
...
- (NSDictionary*)notificationUserInfo {
    return (self.statusLabel.text ? @{DCProgressHUDStatusUserInfoKey : self.statusLabel.text} : nil);
}
...
- (void)controlViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:DCProgressHUDDidReceiveTouchEventNotification
                                                        object:self
    ...
    if(CGRectContainsPoint(self.hudView.frame, touchLocation)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:DCProgressHUDDidTouchDownInsideNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
    }
}
...
- (void)showProgress:(float)progress status:(NSString*)status {
    __weak DCProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong DCProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
...
//                strongSelf.indefiniteAnimatedView.hidden = YES;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.progress = DCProgressHUDUndefinedProgress;
            [strongSelf cancelRingLayerAnimation];
            [strongSelf cancelIndefiniteAnimatedViewAnimation];
...
- (void)showStatus:(NSString*)status {
    ...
    if(self.defaultMaskType != DCProgressHUDMaskTypeNone) {
    ...
    if(self.hudView.alpha != 1.0f){
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:DCProgressHUDWillAppearNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
        ...
        if (self.defaultStyle != DCProgressHUDStyleCustom && !greateriOS9) {
            [self addBlur];
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            if(self.hudView.alpha == 1.0f){
                ...
                // Post notification to inform user
                [[NSNotificationCenter defaultCenter] postNotificationName:DCProgressHUDDidAppearNotification
                                                                    object:self
                                                                  userInfo:[self notificationUserInfo]];
            }
...
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(DCProgressHUDDismissCompletion)completion {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            // Post notification to inform user
            [[NSNotificationCenter defaultCenter] postNotificationName:DCProgressHUDWillDisappearNotification
                                                                object:nil
            ...
            __block void (^completionBlock)(void) = ^{
                ...
                if(strongSelf.hudView.alpha == 0.0f){
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:DCProgressHUDDidDisappearNotification
                                                                        object:strongSelf
                                                                      userInfo:[strongSelf notificationUserInfo]];
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == DCProgressHUDAnimationTypeFlat){
        // Check if spinner exists and is an object of different class
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[DCIndefiniteAnimatedView class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[DCIndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        DCIndefiniteAnimatedView *indefiniteAnimatedView = (DCIndefiniteAnimatedView*)_indefiniteAnimatedView;
...
- (UIColor*)foregroundColorForStyle {
    if(self.defaultStyle == DCProgressHUDStyleLight) {
        return [UIColor blackColor];
    } else if(self.defaultStyle == DCProgressHUDStyleDark) {
...
- (UIColor*)backgroundColorForStyle {
//    return [UIColor colorWithRed:186/255.0f green:186/255.0f blue:186/255.0f alpha:0.7f];
    
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
    // On iOS 8 and DCProgressHUDStyleLight / DCProgressHUDStyleDark the
    // the background color is set via a UIVisualEffectsView
    return self.defaultStyle == DCProgressHUDStyleCustom ? self.backgroundColor : [UIColor clearColor];
#else
    if(self.defaultStyle == DCProgressHUDStyleLight) {
        return [UIColor whiteColor];
    } else if(self.defaultStyle == DCProgressHUDStyleDark) {
...
-(UIView *)backgroundView {
    ...
    // Update styling
    if(self.defaultMaskType == DCProgressHUDMaskTypeGradient){
        ...
        if(self.defaultMaskType == DCProgressHUDMaskTypeBlack){
            _backgroundView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.4];
        } else if(self.defaultMaskType == DCProgressHUDMaskTypeCustom){
...
{
    // Add blur effects
    UIBlurEffectStyle blurEffectStyle = self.defaultStyle == DCProgressHUDStyleDark ? UIBlurEffectStyleDark : UIBlurEffectStyleExtraLight;
...
- (void)setDefaultAnimationType:(DCProgressHUDAnimationType)animationType {
